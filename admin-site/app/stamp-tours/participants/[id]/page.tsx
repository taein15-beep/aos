"use client";
import LiveParticipantDetail from "./LiveParticipantDetail";

import { useState } from "react";
import {
  AlertTriangle, Ban, CheckCircle2, Circle, ClipboardCheck,
  CalendarDays, Clock3, FilePenLine, Gift, MapPin, MessageSquareText, Pencil,
  Phone, QrCode, ShieldCheck, Trophy, UserCheck, UserRound,
} from "lucide-react";

const menu=[
  {icon:"▦",label:"대시보드"},{icon:"◇",label:"상품관리",children:["상품목록","상품등록","일정표관리","요금관리"]},{icon:"▤",label:"예약관리",children:["예약접수현황","예약달력"]},{icon:"₩",label:"결제관리",children:["결제현황","취소/환불"]},{icon:"⇄",label:"정산관리",children:["정산현황","판매점정산","공급사정산"]},{icon:"♙",label:"회원관리",children:["웹회원","제휴여행사","판매점관리","관리자/직원","그룹/권한"]},{icon:"⌂",label:"거래처관리"},{icon:"qr",label:"스탬프투어 관리",children:["스탬프투어 목록","관광지 관리","경품관리","참여자·진행현황","인증 이력","완주·경품 관리","통계"]},{icon:"▥",label:"통계관리"},{icon:"◎",label:"운영관리",children:["팝업관리","알림관리","알림톡"]},{icon:"⚙",label:"시스템설정",children:["홈페이지설정","결제설정","기본설정"]},
];
...
const actionCopy:Record<ActionKey,[string,string,string]>={
  "정보 수정":["참여자 정보 수정 확인","샘플 참여자 참여자의 기본정보 수정 화면을 시연합니다.","변경 내용과 처리 사유는 관리자 활동 이력에 기록됩니다."],
...
export default function ParticipantDetailPage(){
  ...
  return <div className={`app-shell ${collapsed?"is-collapsed":""}`}>
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumb"><span>스탬프투어 관리</span><b>/</b><span>참여자·진행현황</span><b>/</b><strong>참여자 상세</strong></div><div className="top-actions"><label className="search"><span>⌕</span><input aria-label="빠른 검색" placeholder="예약번호, 고객명, 상품명 검색"/><kbd>⌘ K</kbd></label><button className="icon-btn" onClick={()=>act("업무지원센터를 엽니다.")}>?</button><div className="dropdown-wrap"><button className="icon-btn notice" aria-label="알림" onClick={()=>{setNoticeOpen(!noticeOpen);setProfileOpen(false)}}>♢<i>5</i></button>{noticeOpen&&<div className="dropdown notice-menu"><div className="drop-head"><strong>알림</strong><button onClick={()=>setNoticeOpen(false)}>모두 읽음</button></div><button><span className="alert-dot warn"/><span>오늘 처리할 참여자 업무 3건이 있습니다.<small>방금 전</small></span></button></div>}</div><div className="divider"/><div className="dropdown-wrap"><button className="profile" onClick={()=>{setProfileOpen(!profileOpen);setNoticeOpen(false)}}><span className="avatar">장</span><span><b>애비아넥스트</b><small>관리자 장윤호</small></span><em>⌄</em></button>{profileOpen&&<div className="dropdown profile-menu"><button>내 정보</button><button>환경설정</button><hr/><button className="logout">로그아웃</button></div>}</div></div></header>
      <main className="content participant-detail-content"><LiveParticipantDetail/>
        <section className="panel participant-detail-hero"><div className="participant-identity"><span className="participant-avatar"><UserRound size={23}/></span><div><div className="participant-code-row"><span>STP-20260815-01248</span><span className={`badge ${restricted?"danger":"success"}`}>{restricted?"참여 제한":"완주"}</span><span className="reward-badge applied"><Gift size={11}/>신청 완료</span></div><h1>샘플 참여자</h1><p>철원 DMZ 평화관광 스탬프투어 · 인증 5/8곳</p></div></div><div className="participant-detail-actions"><button className="secondary" onClick={()=>window.location.assign("/stamp-tours/participants")}>목록</button><button className="secondary" onClick={()=>openAction("정보 수정")}><Pencil size={14}/>정보 수정</button><button className="participant-restrict-button" onClick={()=>openAction("참여 제한")}><Ban size={14}/>{restricted?"제한 설정됨":"참여 제한"}</button></div></section>
        <section className="panel participant-info-panel"><div className="panel-head"><div><h2>기본정보</h2><p>업무 처리에 필요한 참여자 정보와 동의 현황입니다.</p></div><span className="privacy-note"><ShieldCheck size={13}/>개인정보 보호 적용</span></div><div className="participant-info-grid">{([['이름','샘플 참여자',UserRound],['휴대전화번호','010-0000-0000',Phone],['최초 참여일','2026.08.15 09:58',CalendarDays],['최근 인증일','2026.08.17 15:36',Clock3],['본인인증 여부','인증 완료',UserCheck],['개인정보 동의일','2026.08.15 09:57',ShieldCheck],['위치정보 동의일','2026.08.15 09:57',MapPin],['완주일','2026.08.17 15:37',Trophy],['경품 신청상태','신청 완료 · 2026.08.17 16:02',Gift]] as const).map(([label,value,Icon])=><div key={label}><span><Icon size={13}/>{label}</span><strong>{value}</strong></div>)}</div></section>
        <section className="panel participant-progress-panel"><div className="panel-head"><div><h2>진행현황</h2><p>관광지별 인증 상태와 위치 확인 결과입니다.</p></div><div className="progress-summary"><span style={{fontSize:11}}><b style={{fontSize:17}}>5</b>/8곳 인증</span><strong style={{fontSize:10}}>완주조건 충족</strong></div></div><div className="participant-progress-overview"><div><span style={{fontSize:11}}>전체 8곳 중 5곳 인증</span><b style={{fontSize:14}}>62.5%</b></div><i><em style={{width:"62.5%"}}/></i><p style={{fontSize:10}}><Trophy size={13}/>완주조건: 전체 8곳 중 5곳 이상 인증</p></div><div className="stamp-grid" style={{gridTemplateColumns:"repeat(6, minmax(0, 1fr))"}}>{spots.map((spot,index)=><article className={`stamp-card ${spot.verified?"verified":"pending"}`} key={spot.code}><div className="stamp-card-head"><span>{spot.verified?<CheckCircle2 size={17}/>:<Circle size={17}/>}</span><div><small style={{fontSize:9}}>{spot.code}</small><strong style={{fontSize:11}}>{spot.name}</strong></div><em style={{fontSize:9}}>{spot.verified?"인증":"미인증"}</em></div><dl><div><dt style={{fontSize:9}}>인증 일시</dt><dd style={{fontSize:10}}>{spot.date}</dd></div><div><dt style={{fontSize:9}}>인증 위치</dt><dd style={{fontSize:10}} className={spot.verified?"normal":"muted"}>{spot.verified&&<MapPin size={11}/>} {spot.verified?"정상":"-"}</dd></div></dl><button style={{fontSize:10}} onClick={()=>setSpotDetail(spot)} disabled={!spot.verified}>{spot.verified?"인증 상세보기":"인증 전"}</button>{spot.verified&&<span className="stamp-number">{index+1}</span>}</article>)}</div></section>
        <div className="participant-bottom-grid" style={{gridTemplateColumns:"repeat(2, minmax(0, 1fr))"}}><section className="panel participant-activity-panel"><div className="panel-head"><div><h2>활동 이력</h2><p>참여 등록부터 관리자 처리까지 시간순으로 표시합니다.</p></div><span className="history-count">최근 7건</span></div><div className="participant-history">{activities.map(([date,title,detail,actor,type],index)=><div className={`history-row ${type}`} key={`${date}-${title}`}><i>{type==="warn"?<AlertTriangle size={13}/>:type==="gift"?<Gift size={13}/>:type==="complete"?<Trophy size={13}/>:type==="admin"?<FilePenLine size={13}/>:type==="join"?<UserCheck size={13}/>:<QrCode size={13}/>}</i><div><span><b style={{fontSize:11}}>{title}</b><small style={{fontSize:9}}>{actor}</small></span><p style={{fontSize:10}}>{detail}</p></div><time style={{fontSize:10}}>{date}</time>{index<activities.length-1&&<em/>}</div>)}</div></section>
    ...
    {action&&<div className="modal-backdrop" role="dialog" aria-modal="true"><div className="participant-action-modal"><div className="modal-head"><div><span className={action==="참여 제한"||action==="인증 취소"?"danger":"info"}>{action==="참여 제한"?<Ban size={17}/>:action==="인증 취소"?<AlertTriangle size={17}/>:<ClipboardCheck size={17}/>}</span><h3>{actionCopy[action][0]}</h3></div><button onClick={closeAction} aria-label="닫기">×</button></div><div className="participant-action-body"><p>{actionCopy[action][1]}</p><div className="action-target"><span>처리 대상</span><b>샘플 참여자 · STP-20260815-01248</b></div><label><span>처리 사유 <b>*</b></span><textarea value={reason} onChange={event=>{setReason(event.target.value);setReasonError(false)}} placeholder="처리 사유를 구체적으로 입력해 주세요." autoFocus/><small className={reasonError?"error":""}>{reasonError?"처리 사유를 입력해 주세요.":"입력한 사유는 관리자 활동 이력에 남습니다."}</small></label><div className="action-warning"><AlertTriangle size={13}/><span>{actionCopy[action][2]}</span></div></div><div className="modal-actions"><button className="secondary" onClick={closeAction}>취소</button><button className={action==="참여 제한"||action==="인증 취소"?"danger-button":"primary"} onClick={confirmAction}>확인 후 처리</button></div></div></div>}
    {spotDetail&&<div className="modal-backdrop" role="dialog" aria-modal="true"><div className="participant-spot-modal"><div className="modal-head"><div><span className="info"><QrCode size={17}/></span><h3>인증 상세</h3></div><button onClick={()=>setSpotDetail(null)} aria-label="닫기">×</button></div><div className="spot-detail-body"><div className="spot-detail-status"><CheckCircle2 size={25}/><span><b style={{fontSize:13}}>정상 인증</b><small style={{fontSize:10}}>QR 및 GPS 위치 확인 완료</small></span></div><dl><div><dt style={{fontSize:10}}>참여자</dt><dd style={{fontSize:11}}>샘플 참여자</dd></div><div><dt style={{fontSize:10}}>관광지</dt><dd style={{fontSize:11}}>{spotDetail.name}</dd></div><div><dt style={{fontSize:10}}>관광지 코드</dt><dd style={{fontSize:11}}>{spotDetail.code}</dd></div><div><dt style={{fontSize:10}}>인증 일시</dt><dd style={{fontSize:11}}>{spotDetail.date}</dd></div><div><dt style={{fontSize:10}}>위치 확인</dt><dd style={{fontSize:11}}>허용 반경 100m 이내 · 정상</dd></div><div><dt style={{fontSize:10}}>인증 방식</dt><dd style={{fontSize:11}}>QR코드 + GPS</dd></div></dl></div><div className="modal-actions"><button className="secondary" style={{fontSize:15}} onClick={()=>setSpotDetail(null)}>닫기</button><button className="secondary" style={{fontSize:15}} onClick={()=>{setSpotDetail(null);openAction("인증 취소")}}>인증 취소</button></div></div></div>}
