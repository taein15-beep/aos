...
export default function StampTourForm({mode="create"}:{mode?:"create"|"edit"}) {
  ...
  const errors={name:submitted&&!name.trim(),region:editing&&submitted&&!region,department:submitted&&!department,manager:submitted&&!manager,startDate:submitted&&!startDate,endDate:submitted&&!endDate,dateOrder:submitted&&!!startDate&&!!endDate&&startDate>endDate,requiredCount:submitted&&completion==="count"&&(!requiredCount||Number(requiredCount)<1),privacy:submitted&&!privacy.trim(),location:submitted&&useLocation&&!locationPolicy.trim()};
  ...
  const rewardGridStyle:React.CSSProperties={display:"grid",gridTemplateColumns:"1fr 1.8fr 1fr 1fr 1.55fr",gap:10,minWidth:1180,overflow:"visible"};
  const rewardCardStyle:React.CSSProperties={minWidth:0,minHeight:96,padding:13,border:"1px solid #d9e1ea",borderRadius:7,background:"#fff",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10};

  return <div className={`app-shell ${collapsed?"is-collapsed":""} ${editing?"":"stamp-tour-create-mode"}`}>
    ...
    <div className="workspace"><header className="topbar"><div className="breadcrumb"><span>스탬프투어 관리</span><b>/</b><span>스탬프투어 목록</span><b>/</b><strong>{editing?"스탬프투어 수정":"스탬프투어 등록"}</strong></div><div className="top-actions"><label className="search"><span>⌕</span><input aria-label="빠른 검색" placeholder="예약번호, 고객명, 상품명 검색"/><kbd>⌘ K</kbd></label><button className="icon-btn" onClick={()=>act("업무지원센터를 엽니다.")}>?</button><div className="dropdown-wrap"><button className="icon-btn notice" aria-label="알림" onClick={()=>{setNoticeOpen(!noticeOpen);setProfileOpen(false)}}>♢<i>5</i></button>{noticeOpen&&<div className="dropdown notice-menu"><div className="drop-head"><strong>알림</strong><button onClick={()=>setNoticeOpen(false)}>모두 읽음</button></div><button><span className="alert-dot info"></span><span>새 스탬프 인증이 접수되었습니다.<small>방금 전</small></span></button></div>}</div><div className="divider"/><div className="dropdown-wrap"><button className="profile" onClick={()=>{setProfileOpen(!profileOpen);setNoticeOpen(false)}}><span className="avatar">장</span><span><b>애비아넥스트</b><small>관리자 장윤호</small></span><em>⌄</em></button>{profileOpen&&<div className="dropdown profile-menu"><button>내 정보</button><button>환경설정</button><hr/><button className="logout">로그아웃</button></div>}</div></div></header>
      ...
      <main className="content stamp-form-content"><section className="page-head stamp-form-page-head"><div><h1>{editing?"스탬프투어 수정":"스탬프투어 등록"}</h1><p>운영 기간과 인증·완주 조건을 설정합니다.</p></div><div className="top-form-actions">{actionButtons}</div></section>
        ...
        {!editing?<>
        <section className="panel form-section"><header><span className="section-number">1</span><div><h2>기본정보</h2><p>스탬프투어 정보와 참여 운영 기준을 설정합니다.</p></div></header><div className="form-section-body create-basic-grid">
          <label className={`form-field ${errors.name?"invalid":""}`}><FieldLabel required>투어명</FieldLabel><input value={name} onChange={e=>setName(e.target.value)} placeholder="예: 철원 DMZ 평화관광 스탬프투어" maxLength={60}/>{errors.name?<small className="error-text">투어명을 입력해 주세요.</small>:<small>모바일 스탬프북에 표시됩니다. ({name.length}/60)</small>}</label>
          ...
          <div className="create-operation-row">
            <label className={`form-field operation-date ${errors.startDate||errors.dateOrder?"invalid":""}`}><FieldLabel required>시작일</FieldLabel><div className="icon-input"><CalendarDays size={14}/><input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)}/></div></label>
            <label className={`form-field operation-date ${errors.endDate||errors.dateOrder?"invalid":""}`}><FieldLabel required>종료일</FieldLabel><div className="icon-input"><CalendarDays size={14}/><input type="date" value={endDate} onChange={e=>setEndDate(e.target.value)}/></div></label>
            <div className="form-field operation-time"><FieldLabel>운영시간</FieldLabel><div className="time-range"><input type="time" value={startTime} onChange={e=>setStartTime(e.target.value)}/><em>~</em><input type="time" value={endTime} onChange={e=>setEndTime(e.target.value)}/></div></div>
            <label className="form-field operation-status"><FieldLabel required>운영상태</FieldLabel><select value={status} onChange={e=>setStatus(e.target.value)}><option>운영 예정</option><option>운영 중</option><option>일시중지</option><option>운영 종료</option></select></label>
            <div className="form-field operation-visibility"><FieldLabel>공개 여부</FieldLabel><div className="toggle-row"><Toggle checked={isPublic} onChange={setIsPublic} label="공개 여부"/><span><b>{isPublic?"공개":"비공개"}</b></span></div></div>
            <label className="form-field operation-image"><FieldLabel>대표 이미지</FieldLabel><div className="compact-image-upload"><FileImage size={15}/><strong>{imageName||"이미지 선택"}</strong><input type="file" accept="image/png,image/jpeg" onChange={e=>setImageName(e.target.files?.[0]?.name||"")}/><span>파일 선택</span></div></label>
          </div>

          <div className="create-copy-row">
            <label className="form-field"><FieldLabel>모바일 스탬프북 소개문구</FieldLabel><textarea rows={5} value={intro} onChange={e=>setIntro(e.target.value)} maxLength={120} placeholder="참여자에게 보여줄 짧은 소개문구를 입력하세요."/><small>목록과 스탬프북 상단에 표시됩니다. ({intro.length}/120)</small></label>
            <label className="form-field"><FieldLabel>상세 안내</FieldLabel><textarea rows={5} value={detail} onChange={e=>setDetail(e.target.value)} placeholder="참여방법, 유의사항, 문의처 등 상세 안내를 입력하세요."/><small>모바일 화면에서 읽기 쉽도록 문단을 나누어 작성해 주세요.</small></label>
          </div>
        </div></section>

        <section className="panel form-section"><header><span className="section-number">2</span><div><h2>참여자 및 완주조건 설정</h2><p>현장 인증 기준과 완주 인정 조건을 설정합니다.</p></div></header><div className="form-section-body create-participation-body">
          <div className="create-participant-row">
            <div className="compact-setting"><FieldLabel>1인 1회 참여</FieldLabel><Toggle checked={oneEntry} onChange={setOneEntry} label="1인 1회 참여"/></div>
            <div className="compact-setting"><FieldLabel>동일 관광지 중복 인증 불가</FieldLabel><Toggle checked={noDuplicateSpot} onChange={setNoDuplicateSpot} label="동일 관광지 중복 인증 불가"/></div>
            <div className="compact-setting"><FieldLabel>위치정보 확인 사용</FieldLabel><Toggle checked={useLocation} onChange={setUseLocation} label="위치정보 확인 사용"/></div>
            <label className={`compact-radius ${useLocation?"":"disabled"}`} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}><FieldLabel>관광지 인증 허용 반경</FieldLabel><select style={{width:90,flex:"0 0 90px"}} value={radius} disabled={!useLocation} onChange={e=>setRadius(e.target.value)}><option value="100">100m</option><option value="200">200m</option><option value="300">300m</option></select></label>
          </div>
          <div className="create-completion-row">
            <label className={`compact-choice ${completion==="all"?"selected":""}`}><input type="radio" name="completion" checked={completion==="all"} onChange={()=>setCompletion("all")}/><span className="radio-mark">{completion==="all"&&<Check size={13}/>}</span><strong>전체 관광지 인증</strong></label>
            <label className={`compact-choice ${completion==="count"?"selected":""}`}><input type="radio" name="completion" checked={completion==="count"} onChange={()=>setCompletion("count")}/><span className="radio-mark">{completion==="count"&&<Check size={13}/>}</span><strong>지정 개수 이상 인증</strong></label>
            <label className={`form-field required-count-field ${completion==="count"?"":"disabled"} ${errors.requiredCount?"invalid":""}`}><FieldLabel required>완주 필요 개수</FieldLabel><div><input type="number" min="1" disabled={completion!=="count"} value={requiredCount} onChange={e=>setRequiredCount(e.target.value)} placeholder="예: 5"/><b>곳</b></div>{completion==="count"&&<small className={errors.requiredCount?"error-text":""}>{errors.requiredCount?"완주에 필요한 관광지 개수를 입력해 주세요.":`적용 예시: 전체 8곳 중 ${requiredCount||"5"}곳 이상 인증 시 완주`}</small>}</label>
          </div>
        </div></section>
        </>:<>
        ...
        {!editing?<section className="panel form-section reward-card-section"><header><span className="section-number">5</span><div><h2>경품 설정</h2><p>완주자의 경품 신청 방식과 수집 정보를 설정합니다.</p></div><div className="reward-header-toggle"><span>{rewardEnabled?"사용":"미사용"}</span><Toggle checked={rewardEnabled} onChange={setRewardEnabled} label="경품 신청 사용"/></div></header><div className="reward-card-scroll"><div className={`form-section-body reward-card-grid ${rewardEnabled?"":"disabled"}`} style={rewardGridStyle}>
          <div className="reward-option-card" style={rewardCardStyle}><div><FieldLabel>완주 즉시 신청 가능</FieldLabel><small>완주 처리와 동시에 신청 버튼을 표시합니다.</small></div><Toggle checked={immediateReward} onChange={setImmediateReward} label="완주 즉시 신청" disabled={!rewardEnabled}/></div>
          <div className="reward-option-card reward-period-card" style={{...rewardCardStyle,display:"grid",alignContent:"center"}}><div><FieldLabel>경품 신청기간</FieldLabel><small>비워두면 투어 종료일까지 신청 가능합니다.</small></div><div className="compact-date-range"><input type="date" disabled={!rewardEnabled} value={rewardStart} onChange={e=>setRewardStart(e.target.value)}/><em>~</em><input type="date" disabled={!rewardEnabled} value={rewardEnd} onChange={e=>setRewardEnd(e.target.value)}/></div></div>
          <div className="reward-option-card" style={rewardCardStyle}><div><FieldLabel>중복 신청 불가</FieldLabel><small>같은 투어 경품은 한 번만 신청할 수 있습니다.</small></div><Toggle checked={noDuplicateReward} onChange={setNoDuplicateReward} label="중복 신청 불가" disabled={!rewardEnabled}/></div>
          <div className="reward-option-card" style={rewardCardStyle}><div><FieldLabel>배송지 수집</FieldLabel><small>경품 배송을 위한 주소를 수집합니다.</small></div><Toggle checked={shipping} onChange={setShipping} label="배송지 수집" disabled={!rewardEnabled}/></div>
          <label className="reward-option-card reward-notice-card" style={{...rewardCardStyle,display:"grid",alignContent:"center",justifyContent:"stretch"}}><FieldLabel>경품 안내문</FieldLabel><textarea rows={2} disabled={!rewardEnabled} value={rewardNotice} onChange={e=>setRewardNotice(e.target.value)} placeholder="경품 구성, 지급 일정, 유의사항을 입력하세요."/></label>
        </div></div></section>:<section className="panel form-section"><header><span className="section-number">5</span><div><h2>경품 설정</h2><p>완주자의 경품 신청 방식과 수집 정보를 설정합니다.</p></div></header><div className="form-section-body settings-list">
