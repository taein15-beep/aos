  ...
  return <div className={`app-shell ${collapsed ? "is-collapsed" : ""}`}>
    <aside className="sidebar">
      ...
      <nav aria-label="관리자 메뉴">{menu.map((item)=><div className="nav-group" key={item.label}>
        ...
        {item.children&&expanded.includes(item.label)&&!collapsed&&<div className="subnav">{item.children.map((child)=><button className={child==="상품목록"?"current":""} key={child} data-planned-path={item.label === "스탬프투어 관리" ? `/stamp-tours/${({"관광지 관리":"attractions","참여자·진행현황":"participants","인증 이력":"verifications","완주·경품 관리":"rewards","통계":"statistics"} as Record<string,string>)[child] || ""}` : undefined} onClick={()=>child === "상품목록" ? window.location.assign("/products") : child === "스탬프투어 목록" ? window.location.assign("/stamp-tours") : child === "관광지 관리" ? window.location.assign("/stamp-tours/attractions") : act(`${child} 화면은 다음 단계에서 제공될 예정입니다.`)}>{child}</button>)}</div>}
      </div>)}</nav>
